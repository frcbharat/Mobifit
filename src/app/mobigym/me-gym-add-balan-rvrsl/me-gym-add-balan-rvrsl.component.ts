        ...
        if (result.value) {
          ...
          this.callHttpPost
            ...
            .subscribe(response => {
              ...
              console.warn(
                "%c ___________________________ Collect Advance Post Response RESPONSE ___________________________",
                "background: #4dd0e1;color: black; font-weight: bold;"
              
              );

              console.log(response);
              console.log('response[0].p_out_mssg_flg',response[0].p_out_mssg_flg);

              // swal.close();

              // Check reponse for success or failure - start
              if ((response[0].p_out_mssg_flg = "S")) {
                // swal ~ start -----
                swal({
                  type: "success",
