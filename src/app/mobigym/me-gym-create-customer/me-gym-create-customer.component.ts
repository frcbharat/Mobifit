...
import { MeCalculatePackPriceService } from 'src/app/Service/me-calculate-pack-price.service';
...
//hiding amount & paymentmode

selectDiscount                      : Boolean = true;
...
// Image Upload Variables
    ...
    onSubmit(value: string): void {
        ...
        */
        ...
        console.log('customer details',this.customer_details);
...
this.sub_total =(this.postRequestObject.records[0].subscription_summary.total_prepaid_amount + this.previousDue ) - this.postRequestObject.records[0].customer_details.discount ;
//(this.postRequestObject.records[0].subscription_summary.total_prepaid_amount + this.previousDue ) - this.postRequestObject.records[0].customer_details.discount
 console.log("prepaid_amount" ,this.postRequestObject.records[0].subscription_summary.total_prepaid_amount);
 console.log("customerBalance" ,this.previousDue);
 console.log("DISCOUNT" ,this.postRequestObject.records[0].customer_details.discount);
console.log("subtotal" ,this.sub_total);



let amtbal: any = this.paymentArray.filter(
    (obj: any) => {
        return obj.payment_type_id != '4'
    }
)
    let totalamt=0;
    console.log('amtbal',amtbal);
// looping the amount value to add

for (var i=0; i<amtbal.length; i++) {

   console.log('amtbal',amtbal[i]);
    totalamt=totalamt+amtbal[i].payment_amount;
...
console.log("payment_tax",this.calculatetaxPrice.base_price);
console.log("payment_tax",this.calculatetaxPrice.cgst_amount);
console.log("payment_tax",this.calculatetaxPrice.sgst_amount);
console.log("payment_tax",this.calculatetaxPrice.total_amount);


//overdue
this.over_due =(this.sub_total - this.calculatetaxPrice.total_amount);
console.log("overdue" ,this.over_due);

console.log("payment array",amtbal);
                    ...
                    // totalamt         = this.calculatetaxPrice.base_price;
    ...
    addPrepaidSubscriptionDetails() {
        ...
        let selected_prepaid_pack_obj = {
            ...
            'total_amount'              : null,
            'extention_period'          : null,
            //changes to display date in invoice
        ...
        this.payment_tax  = this.pack_obj[0].pack_price_type;
...
//     //****************************************************End************************************************** */
    ...
    downloadPdf() { 
        ...
        doc.setFontType('normal')
        doc.setFontSize(10)
       // doc.text(`${(this.postRequestObject.records[0].subscription_summary.total_prepaid_amount + this.previousDue ) - this.postRequestObject.records[0].customer_details.discount.toFixed(2)}`,550,first+54,null,null,'right')
      ...
      //  doc.text(`${this.postRequestObject.records[0].subscription_summary.total_prepaid_cgst_amount.toFixed(2)}`,550,first+94,null,null,'right')
       ...
       doc.text(`${ this.calculatetaxPrice.base_price .toFixed(2)}`,550,first+74,null,null,'right')
       doc.text(`${ this.calculatetaxPrice.cgst_amount .toFixed(2)}`,550,first+94,null,null,'right')
       doc.text(`${ this.calculatetaxPrice.sgst_amount .toFixed(2)}`,550,first+114,null,null,'right')
      
       // doc.text(`${this.postRequestObject.records[0].subscription_summary.total_prepaid_amount.toFixed(2)}`,550,first+74,null,null,'right')
        doc.text(`${this.postRequestObject.records[0].customer_details.discount.toFixed(2)}`,550,first+14,null,null,'right')
        doc.text(`${this.previousDue.toFixed(2)}`,550,first+34,null,null,'right')
        doc.text(`${this.over_due.toFixed(2)}`,550,first+134,null,null,'right')
        doc.setFontSize(11)
        doc.text('Discount',382,first+14)
        doc.text('Previous Due',359,first+34)
        doc.text('Sub-Total',377,first+54)
        doc.text('Base Price',377,first+74)
        ...
        doc.setFontSize(15);
        doc.setFontType('bold')
        doc.text('Total',385,first+160)
        doc.setFillColor(0, 0, 0)
        doc.rect(429.5,first+145,125.5,30,'F')
        doc.setTextColor(255,255,255)
        doc.text(`${this.calculatetaxPrice.total_amount.toFixed(2)}`,550,first+160,null,null,'right')
