...
//enquiry id validation
      this.enquiry_num.valueChanges.subscribe(
        (value : string) => {
            let enquiry_num = this._enquiryNum.find(
                (object : any) => {
                    return object.enquiry_num.toLowerCase() === value.trim().toLowerCase();
                }
            )
            console.log('Enquiry ID ', enquiry_num)

            if(enquiry_num) {
                this.showDuplicate = true;
                console.log('enquiry Id Already Exists')
            } else {
                this.showDuplicate = false;
                console.log('New enquiry Id')
            }
        }
    )
