...
export class MeGymEditCustomerComponent implements OnInit {
    ...
    constructor(
        ...
        //calculating payment amount tax
        public  calculatetaxPrice    : MeCalculatePackPriceService,
...
//
    ...
    ngOnInit() {
        ...
        this.payment_amount.valueChanges.subscribe(
            (value: number) => {
      //changes in tax
                console.log('------ Pack tax Price Type Changed Value ------');
                console.log(value);
                console.log('pack_tax_value',this.pack_tax_value);
                console.log('pack_tax_value',this.payment_details.controls['payment_amount'].value                );

                this.calculatetaxPrice.calculatePackPrice(
                    
                    this.payment_details.controls['payment_amount'].value,
                    this.pack_tax_value,
                );
                console.log(this.calculatetaxPrice.base_price);
                console.log(this.calculatetaxPrice);
...
// this.invoice_ExpiryDate = moment(this.pack_expiry_date.value).format('DD-MMM-YYYY');
// console.log("invoice_ExpiryDate" ,this.pack_expiry_date);
// //activation date
// this.invoice_ActivtationDate =this.packActivationDate;
// console.log("invoice_ActivtationDate" ,this.packActivationDate);
//subtotal

                    this.sub_total =(this.total_prepaid_amount_value + this._customerBalance ) - this.rebateAmount ;
                    
                    console.log("prepaid_amount" ,this.total_prepaid_amount_value);
                    console.log("customerBalance" ,this._customerBalance);
                    console.log("DISCOUNT" ,this.rebateAmount);
                    console.log("subtotal" ,this.sub_total);
//overdue
                    this.over_due =(this.sub_total - this.enteredPaymentAmount);
                    console.log("overdue" ,this.over_due);
                    console.log(
                    ...
                    if (this.prepaidPackArrayNew.length > 0) {
                        ...
                        console.log('this.prepaidPackArrayNew',this.prepaidPackArrayNew);
                    ...
                    this.postRequestObject.records[0].subscription_summary.total_prepaid_base_price         = this.calculatetaxPrice.base_price;
                    ...
                    this.postRequestObject.records[0].subscription_summary.total_prepaid_cgst_amount        = this.calculatetaxPrice.cgst_amount
                    this.postRequestObject.records[0].subscription_summary.total_prepaid_sgst_amount        = this.calculatetaxPrice.sgst_amount
                    ...
                    this.postRequestObject.records[0].subscription_summary.total_prepaid_tax                = this.calculatetaxPrice.total_tax
    ...
    renewPack(pack_id: any, daysRemaining: any) {
        ...
        console.log('********************Selected Pack Object*************************', selected_pack_obj);
            ...
            if(renewPackExists.length > 0) {renewPack
                console.log('*******renewpackexists*********', renewPackExists);
            ...
            selected_pack_obj.pack_id = renewPack[0].pack_id;
            selected_pack_obj.pack_type = renewPack[0].pack_type;
            selected_pack_obj.base_price = this.calculatetaxPrice.base_price;
            selected_pack_obj.cgst_amount = renewPack[0].cgst_amount;
            selected_pack_obj.sgst_amount = renewPack[0].sgst_amount;
            selected_pack_obj.total_tax = renewPack[0].total_tax;
            selected_pack_obj.total_amount = renewPack[0].total_amount;
    ...
    addPrepaidSubscriptionDetails() {
        ...
        let selected_prepaid_pack_obj = {
            ...
            'invoice_ExpiryDate'        :null,
            'invoice_ActivtationDate'     :null,
        ...
        //calc
        this.pack_tax_value = this.pack_obj[0].pack_price_type;
        ...
        selected_prepaid_pack_obj.invoice_ActivtationDate = moment(this.packActivationDate).format('DD-MMM-YYYY');
        selected_prepaid_pack_obj.invoice_ExpiryDate = moment(expiryDate).format('DD-MMM-YYYY');
    ...
    downloadPdf() { 
        ...
        doc.text(`${moment().format("DD-MM-YYYY")}`,475,105)
        ...
        var columns = [
            ...
            { title:"Activation Date",dataKey:"invoice_ActivtationDate"},
            { title:"Expiry Date",dataKey:"invoice_ExpiryDate"},
            { title:"Amount",dataKey:"total_amount"}
        ]
        var res = doc.autoTable(columns, data,{startX:250,startY: 384,theme:'grid',
        columnStyles: { rownum:{columnWidth:40,halign:'center'},pack_name:{columnWidth:130}, invoice_ActivtationDate: {columnwidth: 180} , invoice_ExpiryDate: {columnwidth: 180}, total_amount: {columnWidth: 125,halign:'right'}},
        ...
        doc.rect(430,first+120,125,20)
        doc.setFontType('normal')
        doc.setFontSize(10)
        //doc.text(`${this.postRequestObject.records[0].subscription_summary.total_prepaid_base_price.toFixed(2)}`,550,first+74,null,null,'right')
        doc.text(`${(this.postRequestObject.records[0].subscription_summary.total_prepaid_base_price + this.postRequestObject.records[0].subscription_summary.total_prepaid_cgst_amount + this.postRequestObject.records[0].subscription_summary.total_prepaid_sgst_amount ).toFixed(2)}`,550,first+74,null,null,'right')
        doc.text(`${this.postRequestObject.records[0].subscription_summary.total_prepaid_cgst_amount.toFixed(2)}`,550,first+94,null,null,'right')
        doc.text(`${this.postRequestObject.records[0].subscription_summary.total_prepaid_sgst_amount.toFixed(2)}`,550,first+114,null,null,'right')
        doc.text(`${this.sub_total.toFixed(2)}`,550,first+54,null,null,'right')
      //this.payment_details.controls['payment_amount'].value
        doc.text(`${this.postRequestObject.records[0].payment_details.rebate_amount.toFixed(2)}`,550,first+34,null,null,'right')
        doc.text(`${this._customerBalance.toFixed(2)}`,550,first+14,null,null,'right')
        doc.text(`${this.over_due.toFixed(2)}`,550,first+134,null,null,'right')
        doc.setFontSize(11)
        doc.text('Previous Due',359,first+14)
        doc.text('Discount(-)',370,first+34)
        doc.text('Sub Total',370,first+54)
        doc.text('Paid Amount(Inc Tax)',322,first+74)
        doc.text('CGST',380,first+94)
        doc.text('SGST',380,first+114)
        doc.text('Overdue',378,first+134)
        // doc.text('Monthly Charge',348,first+74)
        
        
        doc.setFontSize(15);
        doc.setFontType('bold')
        doc.text('Total',385,first+170)
        doc.setFillColor(0, 0, 0)
        doc.rect(429.5,first+155,125.5,30,'F')
        doc.setTextColor(255,255,255)
      //  doc.text(`${(this.postRequestObject.records[0].subscription_summary.total_prepaid_amount - this.postRequestObject.records[0].payment_details.rebate_amount - this._customerBalance).toFixed(2)}`,550,first+170,null,null,'right')
      doc.text(`${(this.postRequestObject.records[0].subscription_summary.total_prepaid_base_price + this.postRequestObject.records[0].subscription_summary.total_prepaid_cgst_amount + this.postRequestObject.records[0].subscription_summary.total_prepaid_sgst_amount ).toFixed(2)}`,550,first+170,null,null,'right')
